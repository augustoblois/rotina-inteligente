...
class Tarefa(Atividade):
    ...
    def _exibir_extras(self):
        ...
        if dias < 0:
            print(f"       Prazo: {self._prazo.strftime('%d/%m')} — ATRASADA {abs(dias)} dia(s)")
        elif dias == 0:
            print(f"       Prazo: {self._prazo.strftime('%d/%m')} — HOJE!")
        else:
            print(f"       Prazo: {self._prazo.strftime('%d/%m')} — {dias} dia(s) restante(s)")
...
class HabitoSaude(Atividade):
    ...
    def registrar_realizacao(self):
        self._realizacoes += 1
        print(f"  ✔ '{self._nome}' realizada ({self._realizacoes}/{self._frequencia}x na semana).")
...
class Rotina:
    ...
    def __init__(self):
        self._semana: dict[str, DiaSemana] = {d: DiaSemana(d) for d in self.DIAS}
...
if __name__ == "__main__":
    ...
    # Compromissos fixos
    r.adicionar("Segunda",  Compromisso("Estágio na clínica",    4, 5, "08h–12h", "Clínica Norte"))
    r.adicionar("Segunda",  Compromisso("Aulas de Psicologia",   3, 4, "14h–17h", "Faculdade"))
    r.adicionar("Terça",    Compromisso("Estágio na clínica",    4, 5, "08h–12h", "Clínica Norte"))
    r.adicionar("Terça",    Compromisso("Aulas de Psicologia",   3, 4, "14h–17h", "Faculdade"))
    r.adicionar("Quarta",   Compromisso("Estágio na clínica",    4, 5, "08h–12h", "Clínica Norte"))
    r.adicionar("Quarta",   Compromisso("Aulas de Psicologia",   3, 4, "14h–17h", "Faculdade"))
    r.adicionar("Quinta",   Compromisso("Estágio na clínica",    4, 5, "08h–12h", "Clínica Norte"))
    r.adicionar("Quinta",   Compromisso("Aulas de Psicologia",   3, 4, "14h–17h", "Faculdade"))
    r.adicionar("Sexta",    Compromisso("Estágio na clínica",    4, 5, "08h–12h", "Clínica Norte"))
    r.adicionar("Sexta",    Compromisso("Reunião de pesquisa",   2, 4, "18h–20h", "Online"))

    # Tarefas com prazo
    r.adicionar("Quinta",   Tarefa("Trabalho Psicologia Social", 3, 5, date(2026, 5, 22)))
    r.adicionar("Terça",    Tarefa("Relatório de estágio",       2, 4, date(2026, 5, 27)))
    r.adicionar("Sábado",   Tarefa("Revisão para prova",         3, 4, date(2026, 5, 30)))
